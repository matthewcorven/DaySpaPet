...
using Behavior = DaySpaPet.WebApi.Api.Behavior;
using NJsonSchema;
using NSwag;
...
internal sealed class AddRequestOriginClockTimeZoneId : IOperationProcessor
{
  ...
  public AddRequestOriginClockTimeZoneId(Action<RequestOriginClockTimeProcessorOptions>? configure = null)
  {
    if (configure != null)
    {
      ...
      if (settings.Strict)
      {
        Guard.Against.NullOrEmpty(RuntimeEnvironmentValues.HttpRequestHeader, Behavior.Constants.HttpRequestHeaderKey);
      }
    }
  }
  ...
  {
    var RequestHeader_OriginClockTZDBID = new OpenApiParameter()
    {
      Name = Behavior.Constants.HttpRequestHeaderKey,
